...
class NN2Layer(chainer.Chain):
    def __init__(self):
        w = I.Normal(scale=0.1)
        super(NN2Layer, self).__init__(
                l1 = L.Linear(2, 500, initialW=w),
                l2 = L.Linear(None, 1, initialW=w),
        )
...
#    plot_data_3d(XX, YY, ZZ)
    ...
    #ニューラルネットワークの登録
    model = L.Classifier(NN2Layer(), lossfun=F.mean_squared_error)  #平均2乗誤差
...
#    chainer.serializers.load_npz("result/snapshot_iter_100000", trainer)
